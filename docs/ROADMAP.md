...
Roadmap реализует целевое состояние продукта, описанное в [Product Vision](PRODUCT_VISION.md).

Он построен так, чтобы каждый milestone доказывал новую способность продукта, а не просто добавлял инфраструктуру.

`PRODUCT_VISION.md` отвечает на вопрос **«что мы в итоге строим?»**.
Этот документ отвечает на вопрос **«какими доказуемыми шагами мы туда идём?»**.
